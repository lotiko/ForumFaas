...
router.get("/:catname", async (req, res, next) => {
    if (req.params.catname === "presentation") {
        ...
        try { // execute query with page and limit values
            const users = await User.find().select("-password").limit(limit * 1).skip((page - 1) * limit).exec();
...
router.post("/:catname", (req, res, next) => { // /////////////PRESENTATION
    if (req.params.catname === "presentation") {
        return;
    }
    // /////////////FUNCTION
    if (req.params.catname === "function") {
        ...
        new Function({name: name, args: args, body: body, userId: req.user._id}).save().then((funInDb) => {
    ...
    if (req.params.catname === "answer") {
        ...
        new PostModel({
            title, body, categorie,
            // withFunction,
            userId
        }).save().then(function (answersFromDb) {
...
router.get("/:catname/:id",(req,res,next)=>{
  ...
  .then(answerFromDb=>{res.render('forum/detail/answer',answerFromDb)})
  .catch(err=>next(err))
